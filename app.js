...
async function onRpcConnectionVerified(getnetworkinfo, getblockchaininfo) {
	...
	if (match) {
		...
		debugErrorLog(`Unable to parse node version string: ${getnetworkinfo.subversion} - RPC versioning will likely be unreliable. Is your node a version of Sumcoin Core?`);
	}
	...
	coreApi.getBlockchainInfo().then(function(result) {
		...
		Promise.all(promises).then(function(results) {
			...
			if (results && results.length > 0 && results[0] != null) {
				...
				debugLog("Unable to load network volume, likely due to sumcoind version older than 0.17.0 (the first version to support getblockstats).");
			}
		});
	});
}
...
function connectToRpcServer() {
	...
	// which changes each startup of sumcoind
